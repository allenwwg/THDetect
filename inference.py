    ...
    for j in range(h // step):
        for i in range(w // step):
            ...
            if crop_image.shape[:2] != (target_h, target_h):
                ...
                cnt += 1
    logger.info("patch number:{}".format(len(clip_list)))
    time_1 = time.time()
...
def get_inference_block_mask(bboxes, labels, scores, img_scale, id_start,
    ...
    temp_id = np.zeros(mask_size, np.uint16)
    temp_score = np.zeros(mask_size, np.uint16)
    box_dict = {}
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        if len(bbox) > 4:
            score = bbox[-1]
        # box_dict[id_start] = [
        #     x1 + origin_x, y1 + origin_y, x2 + origin_x, y2 + origin_y, label,
        #     score
        # ]
        ...
        t_score = int(score * 100000 - 40000)
        if t_score < 0:
            continue
        if label == 11:
            t_score = 60000
        ...
        for y in range(y1, y2):
            for x in range(x1, x2):
                if (temp_score[y, x] <= t_score):  # FIXME: 如果是同类别的话就不要更新了
                    temp_score[y, x] = t_score
                    if temp_cat[y, x] == label + 1:
                        union_set.add(temp_id[y, x])
                    temp_cat[y, x] = label + 1  # FIXME:防止0
                    temp_id[y, x] = id_start
                elif temp_cat[y, x] == label + 1:
                    flag=False
        for c in union_set:
            print(c)
            temp_id[temp_id == c] = id_start
        
        box_dict[id_start] = [ x1 + origin_x, y1 + origin_y, x2 + origin_x, y2 + origin_y, label, score   ]
    return temp_cat.copy(), temp_score.copy(), temp_id.copy(), id_start, box_dict
...
def merge_clip_top_and_left(thisclip_ids, mask_category, mask_id, step, thres,
    ...
    def mergeDict(list_a, list_b, trans_a, trans_b):
        x1_a, y1_a, x2_a, y2_a, label_a, score_a = list_a
        x1_b, y1_b, x2_b, y2_b, label_b, score_b = list_b
        ...
        return [x1_c, y1_c, x2_c, y2_c, label_a, score_c]
    ...
    if col_id > 0:
        ...
        cat_set = set()
        for i in range(step):
            if (mask_category[this_y + i, this_x] == mask_category[top_y + i,
                                                                   top_x]
                    and mask_category[this_y + i, this_x] != 0):  # 相交
                temp[0, i] = mask_category[this_y + i, this_x]
                cat_set.add(temp[0, i])
                inter_cnt += 1
            ...
            if length / (last - first + 1) >= thres:
                ...
                for y in range(this_y, this_y + step):
                    for x in range(this_x, this_x + step):
                        if (mask_id[y, x] == this_id
                                and mask_category[y, x] == cat):
                            flag = True
                            mask_id[y, x] = top_id
                if box_dict.get(top_id) is not None and box_dict.get(this_id) is not None:
                    box_dict[top_id] = mergeDict(box_dict[top_id],
                                                box_dict[this_id],
                                                (this_y, this_x - step),
                                                (this_y, this_x))
    ...
    if row_id > 0:
        ...
        cat_set = set()
        for i in range(step):
            if (mask_category[this_y, this_x + i] == mask_category[top_y,
                                                                   top_x + i]
                    and mask_category[this_y, this_x + i] != 0):  # 边缘交集：相交=1
                temp[0, i] = mask_category[this_y, this_x + i]  # FIXME:1
                cat_set.add(temp[0, i])
                inter_cnt += 1
        # print("{},{} : {}".format(row_id,col_id,inter_cnt))
    ...
    return mask_category.copy(), mask_id.copy(), box_dict
...
def clip_for_inference(model,
    ...
    for j in range(h // step):
        for i in range(w // step):
            ...
            if np.sum(clip_list[clip_cnt]) < ALL_WHITE:
                ...
                mask_category[topleft_y:topleft_y + step, topleft_x:topleft_x +
                              step] = temp_cat[margin:margin + step,
                                               margin:margin +
                                               step].copy()  # 赋值中心区域
                mask_score[topleft_y:topleft_y + step, topleft_x:topleft_x +
                           step] = temp_score[margin:margin + step,
                                              margin:margin + step].copy()
                mask_id[topleft_y:topleft_y + step, topleft_x:topleft_x +
                        step] = temp_id[margin:margin + step,
                                        margin:margin + step].copy()
            ...
            predict_png[topleft_y:topleft_y + step, topleft_x:topleft_x +
                        step] = clip_list[clip_cnt][margin:margin + step,
                                                    margin:margin +
    ...
    logger.info("inference time:{}".format(infer_time_sum))
...
def process_hbb(img, id_cnt_list, dict_box, flag=True, scale_para=1,margin=90):
    ...
    for i in range(len(id_cnt_list)):
        ...
        if label == 8 or label == 9:    # 筛选一下淋浴房的尺寸
            if x2 - x1 <= 165 / scale_para or y2 - y1 <= 165 / scale_para:
                continue
        ...
        cv.putText(img, MYCLASSES[label]+ ":{:.2f}".format(score),
...
if __name__ == "__main__":
    ...
    parser.add_argument("-score_thres", type=float, default=0.7)  # 阈值筛选
    ...
    1. cmd环境激活：
        ...
        activate openmmlab
    2. 单例：
        python inference.py -image_name 1 -score_thres 0.7
    3. 多例：
        python inference.py -image_name *  -data_dir D:\\THdetection\\test\\ -res_dir d:\img_result\small\    -json_dir d:\img_label\small\ -score_thres 0.4
        python inference.py -image_name *  -data_dir /home/mjy/mmdetection/myutils/image/ -res_dir /home/mjy/mmdetection/myutils/result/    -json_dir /home/mjy/mmdetection/myutils/label/ -score_thres 0.6
    ...
    config_file = "D:\ProgramData\Anaconda3\mmdetection\workExp\droprate01\mask_rcnn_swin-t-p4-w7_fpn_fp16_ms-crop-3x_coco.py"
    checkpoint_file = "D:\ProgramData\Anaconda3\mmdetection\workExp\droprate01\epoch_36.pth"  
    ...
    for item in tqdm(infer_img_list):
        ...
        clip_list = get_clips(image, window_size, center_size)  # 切分大图为小图片，返回一个list
